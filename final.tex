...
so it is different from biological reactions that involve proteins and other macromolecules. \footnote{Such 
...
\subsection{Related Work.} Process calculi, originally designed for modelling sequential and 
concurrent computation, have been applied to biochemical and biological systems. The main 
instances of that are $\pi$-calculus \cite{Regev2004}, BioAmbients \cite{RegevBioambients}, the stochastic $\pi$-calculus \cite{PriamiStochasticPi}, beta binders~\cite{betabinders} and bioPEPA \cite{CiocchettaBiopepa}. Another way to model biochemical reactions is with rule-based formalisms such as BIOCHAM \cite{biocham}, the $\kappa$-calculus  \cite{danoscausality} and the BioNetGen Language (BNGL) \cite{pmid19399430}. 
...
biochemical reactions in metabolic pathways, gene expression, signal transduction, and neural processes~\cite{PNbiology,DBLP:journals/bib/Chaouiya07,DBLP:journals/nc/BaldanCMS10}. Indeed, PNs seem to be a natural framework for representing biochemical systems as they constitute a set of interdependent
...
\subsection{Paper organization.} In the next section, we introduce the autoprotolysis of water 
...
transitions to break bonds. Thus, a transition may simultaneously create
and destruct bonds, and its reversal results in the opposite effect. Formally, an RPN
is defined as follows:
...
outgoing arcs. 
Graphically, an RPN is portrayed as a directed bipartite graph where token instances are indicated by $\bullet$, places by
...
the following we omit the discussion of negative tokens and negative bonds as they are not relevant to our
case study.  
...
instances. A pair of a marking and a history, $\state{M}{H}$, describes a \emph{state} of an RPN 
...
During the forward execution of a transition in an RPN, 
...
We have compared the Calculus of Covalent Bonding (CCB), the Bonding Calculus, and Reversing Petri Nets as models of chemical reactions and reversible processes in general. We have shown that they can all model the out-of-causal-order reversibility present in such reactions. We have also shown that the calculi allow some reactions which do not happen in reality. This is due to the modelling abstracting away from some real-world properties. On the other hand, some reactions are not possible in the model even though they can happen in reality. This is because the modelling is not general enough. We have also given software packages which allow simulation and analysis for these formalisms.
