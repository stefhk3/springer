...
%
Backtracking executes exactly the reverse order of the 
forward execution, and causally-consistent reversibility 
executes effects before causes, but not necessarily in exact reverse 
...
reversibility~\cite{Irek2012}. A typical example is use of a catalyst $C$, which enables compound 
$A$ and $B$ to combine, which would normally not happen or be very unlikely without the presence 
...
is undone before its effect, namely the bond from $A$ to $B$. 
...
illustrating example of the various types of reversibility refer to~\cite{danos2004ccsr,Irek2007,LaneseMS10,Irek2012}.

\paragraph{Contribution.} This chapter presents and compares three formalisms, the Calculus of Covalent Bonding (CCB) \cite{KUHN201818}, the Bonding Calculus \cite{NaCo18}, and Reversing Petri Nets (RPNs)~\cite{RPNs}, that have been developed during COST Action IC1405. These models are variations of existing formalisms and are set out to study reversible computation by allowing systems to reverse at any time leading to previously visited states or even new ones without the need of additional forward actions. The contribution of this chapter is a comparative overview of the three formalisms, a discussion of their expressiveness, and a demonstration of their use on a common case study, namely the autoprotolysis of water
reaction.
...
interactions y are sometimes not fully understood. 
%This could also mean that some aspects of reversibility are not clearly modelled since  too  much
%detail is abstracted away. 
Therefore,  new modelling techniques may be are needed in order to capture reversible behaviour 
when studying biological systems. In this chapter, however, we present we concentrate on chemical 
...
\paragraph{Related Work.} Process calculi, originally designed for modelling concurrent computations, have been applied to biochemical and biological systems. We can mention the $\pi$-calculus \cite{MilnerPi,Regev2004}, BioAmbients \cite{RegevBioambients}, the stochastic $\pi$-calculus \cite{PriamiStochasticPi}, beta binders~\cite{betabinders} and bioPEPA \cite{CiocchettaBiopepa}. 
...
Attempts at modelling reversibility in process calculi include, amongst others,
RCCS \cite{danos2004ccsr}, CCSK \cite{Irek2007}, and reversible 
$\pi$ \cite{Lanese_controllingreversibility,Lanese_controlled}. CCSK and RCCS are based on the Calculus of Communicating Systems (CCS) \cite{MilnerBook}. They extend CCS by keeping track of past actions 
...
These calculi support backtracking and causally-consistent reversibility. However, the Calculus 
of Covalent Bonding (CCB) \cite{KUHN201818} that we present here enables the modelling of 
...
Since then they have been widely employed for
...
introducing the formalism, their syntax and, informally, their operational semantics. We also 
give there models of the autoprotolysis of water using the formalisms. The conclusion contains 
...
reaction is known as \emph{autoprotolysis of water} and is shown 
...
In order to model the reaction we need to understand its why it takes place and what causes it.
...
a type of explicit reversibility we call ``locally controlled reversibility''. We have modelled a simple covalent chemical reaction in CCB. A corresponding modelling approach can be used to model 
...
The three presented formalisms can model our example chemical reaction fairly well but, as expected, 
...
All of them avoid a $H_3O^+$ molecule forming directly. CCB allows one reaction which is not realistic: If there are many water molecules and therefore several hydroxide and water molecules at the same time, it is possible that the remaining hydrogen is transferred from the hydroxide to a water. In reality, this is not possible since the hydroxide is strongly negatively charged and no hydrogen bond can form.
%
%
{\bf TODO: how about the other formalisms? I would be surprised to hear if they avoid such unwanted 
reactions. So, for a more balanced presentation, we must write something for other formalisms. 
If such unwanted reactions can be avoided there, it would be very interesting to read why. 
One way or the other, please provide a short explanations for the BC and RPNs.}

There are a number of software tools for our formalisms that can aid simulation and analysis. Regarding Bonding Calculus, we can simulate various bonding descriptions by
using an existing software platform called UPPAAL (as shown in [1]). For CCB, there 
is a simulation tool presented in \cite{10.1007/978-3-319-99498-7_8}. It allows much closer form 
of representation of chemical notation than that possible with a typical programming language.
	%As in other formalisms, Petri nets are also associated with a wide range of analysis techniques and the associated tool support, such as determining states of equilibrium, conflicting evolutions, and reachable states.
	 Revering Petri nets have been shown to be closely related to Coloured Petri Nets, as a subset of the former model has been encoded into the latter~\cite{RPNtoCPN}. 
