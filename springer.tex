...
\begin{document}
%
\title{Reversibility in chemical reactions\thanks{The author acknowledges partial support of COST Action IC1405 on Reversible Computation - extending horizons of computing.}}
%
...
Move general things from CCB section.
...
while presenting an intuitive model of the autoprotolysis of water. We also use this to introduce some chemical concepts.
...
produce bonds between atoms. We define $\gamma$ in the style of ACP (\cite{ACPBook}, \cite{BaetenBook}), 
...
Secondly, we can also disregard subscripts (which were introduced to distinguish copies of atoms of the same type). Informally we can say that if we remove the subscripts on the actions and process names and change keys accordingly, then only two processes can be distinguished. In Chapter~\ref{sec:ccbwithsubscripts} we will call this chemical process equivalence. To demonstrate this we use processes 1 and 14. Process 1 is
...
\; \setminus\{h_1,h_2,h_3,o_1,o_2\}$$
So our calculus allows a reaction which does not change the actual state of the system. We have replaced the original key $1$ with a new key $5$, but we will see in Chapter~\ref{sec:ccbwithsubscripts} that this change is not indicating an actual change in the system.
}
\end{example}
